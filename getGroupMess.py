...
newpath = "/mnt/messages/groups"
if not os.path.exists(newpath):
    os.makedirs(newpath)

resp =  requests.get(variables.address+'/groups.listAll', headers=headers,)
if resp.status_code == requests.codes.ok:
    r_all = resp.json()
    for obj in r_all['groups']:
        if not checkIdChata(obj['_id']):
            data = '{ "roomId": "%s", "userId": "%s" }'%(obj['_id'],"C8Edb9d4THyJfwL76")
            data1 = '{ "roomId": "%s" }'%obj['_id']

            resp1 = requests.post(variables.address+'/groups.invite', headers=headers, data=data)
            if not resp1.status_code == requests.codes.ok:
                print(resp1.raise_for_status())
            params = (
                ('roomId', obj['_id']),
            )
            response = requests.get(variables.address+'/groups.messages', headers=headers, params=params)
            ...
            resp2 = requests.post(variables.address+'/groups.leave', headers=headers, data=data1)
